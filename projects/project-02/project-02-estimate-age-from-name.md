...
tbl_lifetables |> 
  ...
  labs(
    title = "Distribution Of People Alive",
    subtitle = "Based On Year They Were Born",
    x = "Year",
    y = "Probability",
    caption = "Source: SSA"
  ) +
...
tbl_lifetables_extended_2022 |> 
 ...
 geom_line () +
  ...
  labs(
    title = "Distribution Of People Alive",
    subtitle = "Extended + Survival Curve",
    x = "Year",
    y = "Probability",
...
tbl_names_extended |> 
  ...
  dplyr::filter(name == "Mirian", sex == "F", between(year, 1930, 1999)) |>
  # Estimate age stats
  estimate_age_stats()
```
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    geom_col(aes(y = nb_alive), fill = 'hotpink', color ='white') +
...
tbl_names_extended_age |> 
  ...
  # Add labels (title, subtitle, x, y)
  labs(
    title = "Median Age of American Females",
    subtitle = "Segments 25th and 75th percentiles",
    x = "Percentiles",
    y = "Names"
  ) +
